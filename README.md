# harmonyos-persianray-core

This folder is a **copy** of `persianray-ios/ios-awg-xray`. Do not edit the iOS library in place. HarmonyOS NEXT builds live here.

- HarmonyOS `c-shared`: [OHOS.md](OHOS.md)
- Script: `MobileLibrary/OHOS/build-go-ohos.sh`
- GitHub Action (monorepo): `.github/workflows/harmonyos-persianray-core.yml`

---

# ios-awg-xray (upstream copy)
...
# → build/AwgXray.xcframework
```

This directory is self-contained repository content: push `ios-awg-xray` as the
repository root, including its `psiphon-ios/` child folder. The build does not
look for a sibling Psiphon checkout.

GitHub Action: `awg-xray-apple`. It expects this directory at the repository
root, validates the customized Psiphon marker files, and uploads
`build/AwgXray.xcframework`. Drop the artifact at
`persianray-ios/Vendor/AwgXray/AwgXray.xcframework` and set
`useUnitedAwgXray = true`.
